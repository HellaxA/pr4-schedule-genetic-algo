...
public class GeneticAlgorithm {
    ...
    Population crossoverPopulation(Population population) {
        ...
        IntStream.range(0, Driver.NUMB_OF_ELITE_SCHEDULE)
                .forEach(x -> crossoverPopulation.getSchedules().set(x, population.getSchedules().get(x)));

        IntStream.range(Driver.NUMB_OF_ELITE_SCHEDULE, population.getSchedules().size()).forEach(x -> {
    ...
    Population mutatePopulation(Population population) {
        ...
        IntStream.range(0, Driver.NUMB_OF_ELITE_SCHEDULE).forEach(x -> schedules.set(x, population.getSchedules().get(x)));
        IntStream.range(Driver.NUMB_OF_ELITE_SCHEDULE, population.getSchedules().size()).forEach(x -> {
